...
# Load dataset
df = pd.read_csv('data4.csv', on_bad_lines='skip')  # Ensure this CSV is in the same folder as the script

# Rename columns to match the expected format
df = df.rename(columns={'Password': 'password', 'Strength': 'strength'})

df['password'] = df['password'].astype(str)

# Map strength strings to numeric values
df['strength'] = df['strength'].replace({'Weak': 0, 'Medium': 1, 'Strong': 2}).astype(int)
