...
# Load dataset
df = pd.read_csv('data3.csv', on_bad_lines='skip')  # Ensure this CSV is in the same folder as the script

df['password'] = df['password'].astype(str)

# Map strength strings to numeric values
df['strength'] = (df['strength']*100).astype(int)

df['strength'] = df['strength'].apply(update_strength)
